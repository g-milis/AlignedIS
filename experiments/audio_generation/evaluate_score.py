def pipeline(output_path, score_save_path, eps, model_str, dataset_name, max_generations=None):
    ...
    task_worker_ = Process(
        ...
        args=(get_in_ds_undetectable_exp, output_path, tq, model_str),
        kwargs={"batch_size": 1, "beta_only": False, "dataset_name":dataset_name, "max_generations": max_generations}
    )

    score_worker_ = [
        Process(
            target=beta_score_worker,
            args=(tq, tqe, rq, i),
            kwargs={"oracle_model_str":model_str,
                    "eps":eps}
        )

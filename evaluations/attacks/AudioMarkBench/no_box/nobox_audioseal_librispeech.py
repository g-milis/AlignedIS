...
def parse_arguments():
    ...
    parser.add_argument("--batch_size", type=int, default=50, help="Maximum length of audio samples to process (in time samples)")
    return parser.parse_args()

def encode_audio_files(model, selected_indices, librispeech_test, output_dir, max_length):
    total_ba = []
    total_probs = []
    total_counts = 0
    
    for idx in tqdm(selected_indices, desc="Encoding Watermarks"):
        waveform, sample_rate, utterance, _, _, _ = librispeech_test[idx]
        file = f"{idx}.wav"
        ...
        # msg = torch.zeros((signal.shape[0], model.msg_processor.nbits), dtype=torch.int64, device=signal.device)
...
def decode_audio_files(model, output_dir, batch_size):
    ...
    original_msgs = []
    path_list = []
    output_txt_dir = f'txt_LibriSpeech_AudioSeal/test'
...
def decode_audio_files_perturb(model, output_dir, common_perturbation, args):
    ...
    if common_perturbation == "time_stretch":
        ...
        for speed_factor in speed_factor_list:
            ...
            total_visqol_scores = []
            total_counts = 0
            output_dir_pert = f'log_audioseal_LibriSpeech_max_5s/common_pert_time_stretch_speed_{speed_factor}'
            ...
            path_list = []
            visqol = api_visqol()
            output_txt_dir = f'txt_LibriSpeech_AudioSeal/no_box_attack'
            os.makedirs(output_txt_dir, exist_ok=True)
        ...
        for snr in snr_values:
            ...
            total_visqol_scores = []
            total_counts = 0
            output_dir_pert = f'log_audioseal_LibriSpeech_max_5s/common_pert_{common_perturbation}_snr_{snr}'
            os.makedirs(output_dir_pert, exist_ok=True)
            file_list = [file for file in os.listdir(output_dir) if file.endswith('.wav')]
            progress_bar = tqdm(enumerate(file_list), desc=f"Applying {common_perturbation}")
        ...
        for nq in n_q_values:
            ...
            total_counts = 0
            # for snr in snr_values:
            output_dir_pert = f'log_audioseal_LibriSpeech_max_5s/common_pert_{common_perturbation}_nq_{nq}'
            os.makedirs(output_dir_pert, exist_ok=True)
            file_list = [file for file in os.listdir(output_dir) if file.endswith('.wav')]
            progress_bar = tqdm(enumerate(file_list), desc=f"Applying {common_perturbation}")
            ...
            with open(output_file, 'w') as txt_file:
                ...
                for id, file in progress_bar:
                    ...
                    payload_str = waveform_pert_file.split('_')[1]
                    original_msg_np = np.array(list(map(int, payload_str)))
                    original_msg = torch.tensor(original_msg_np, dtype=torch.int32).to(device=next(model.parameters()).device)
                    original_msgs.append(original_msg)
                    ...
                    batch_watermarked_signals.append(waveform_pert)
                    if args.save_pert:
                        torchaudio.save(os.path.join(output_dir_pert, file), torch.tensor(waveform_pert), sample_rate)
                    # Extract original payload from filename for BER calculation
                    if len(batch_watermarked_signals) == args.batch_size or id == len(file_list) - 1:
                        batch_watermarked_signals = torch.cat(batch_watermarked_signals, dim=0)
                        original_msgs = torch.stack(original_msgs)
        ...
        for bitrate in bitrate_list:
            ...
            total_visqol_scores = []
            total_counts = 0
            output_dir_pert = f'log_audioseal_LibriSpeech_max_5s/common_pert_{common_perturbation}_bitrate_{bitrate*16}k'
            os.makedirs(output_dir_pert, exist_ok=True)
            file_list = [file for file in os.listdir(output_dir) if file.endswith('.wav')]
            progress_bar = tqdm(enumerate(file_list), desc=f"Applying {common_perturbation}")
        ...
        for bandwidth in bandwidth_values:
            ...
            total_visqol_scores = []
            total_counts = 0
            output_dir_pert = f'log_audioseal_LibriSpeech_max_5s/common_pert_{common_perturbation}_bandwidth_{bandwidth}'
            os.makedirs(output_dir_pert, exist_ok=True)
        ...
        for quantization_bit in quantization_levels:
            ...
            total_visqol_scores = []
            total_counts = 0
            output_dir_pert = f'log_audioseal_LibriSpeech_max_5s/common_pert_{common_perturbation}_quantization_bit_{quantization_bit}'
            os.makedirs(output_dir_pert, exist_ok=True)
            file_list = [file for file in os.listdir(output_dir) if file.endswith('.wav')]
            progress_bar = tqdm(enumerate(file_list), desc=f"Applying {common_perturbation}")
        ...
        for ratio in ratio_list:
            ...
            total_visqol_scores = []
            total_counts = 0
            output_dir_pert = f'log_audioseal_LibriSpeech_max_5s/common_pert_{common_perturbation}_ratio_{ratio}'
            os.makedirs(output_dir_pert, exist_ok=True)
            file_list = [file for file in os.listdir(output_dir) if file.endswith('.wav')]
            progress_bar = tqdm(enumerate(file_list), desc=f"Applying {common_perturbation}")
        ...
        for window_size in window_list:
            ...
            total_visqol_scores = []
            total_counts = 0
            output_dir_pert = f'log_audioseal_LibriSpeech_max_5s/common_pert_{common_perturbation}_window_size_{window_size}'
            os.makedirs(output_dir_pert, exist_ok=True)
            file_list = [file for file in os.listdir(output_dir) if file.endswith('.wav')]
            progress_bar = tqdm(enumerate(file_list), desc=f"Applying {common_perturbation}")
        ...
        for decay in decay_list:
            ...
            total_visqol_scores = []
            total_counts = 0
            output_dir_pert = f'log_audioseal_LibriSpeech_max_5s/common_pert_{common_perturbation}_decay{decay}'
            os.makedirs(output_dir_pert, exist_ok=True)
            file_list = [file for file in os.listdir(output_dir) if file.endswith('.wav')]
            progress_bar = tqdm(enumerate(file_list), desc=f"Applying {common_perturbation}")
        ...
        for bitrate in bitrate_list:
            ...
            total_visqol_scores = []
            total_counts = 0
            output_dir_pert = f'log_audioseal_LibriSpeech_max_5s/common_pert_{common_perturbation}_bitrate_{bitrate}'
            os.makedirs(output_dir_pert, exist_ok=True)
            file_list = [file for file in os.listdir(output_dir) if file.endswith('.wav')]
            progress_bar = tqdm(enumerate(file_list), desc=f"Applying {common_perturbation}")
...
def pert_encodec(waveform, sr, bandwidth, model_encodec, processor):
    model_encodec = model_encodec.to(device)
    if sr != 24000:
        resampler = torchaudio.transforms.Resample(orig_freq=sr, new_freq=24000)
        waveform = resampler(waveform)
    inputs = processor(raw_audio=waveform.squeeze(), sampling_rate=24000, return_tensors="pt").to(device)
    ...
    audio_values = model_encodec.decode(encoder_outputs.audio_codes, encoder_outputs.audio_scales, inputs["padding_mask"])[0].clone().detach().cpu().squeeze().unsqueeze(0)
    if sr != 24000:
        resampler = torchaudio.transforms.Resample(orig_freq=24000, new_freq=sr)
        audio_values = resampler(audio_values)
    return audio_values, sr
...
def pert_mp3(waveform, bitrate, sample_rate=16000):
    mp3_compressor = Mp3Compression(
        ...
        backend="lameenc",  # Choose the backend
        p=1.0  # Set the probability to 1 to always apply the effect
    )
    if waveform.device == "cuda":
...
def pert_xhe_aac(waveform, bitrate_mode=5, sample_rate=16000):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        encoded_m4a = os.path.join(tmpdir, 'compressed.m4a')

        # Save waveform to WAV
        from scipy.io.wavfile import write
        write(input_wav, sample_rate, waveform_np.T)

        # Encode to xHE-AAC
        subprocess.run(['exhale', f'-V{bitrate_mode}', input_wav, encoded_m4a], check=True)

        # Decode with ffmpeg-python
        out, _ = (
            ffmpeg
            .input(encoded_m4a)
            .output('pipe:', format='s16le', acodec='pcm_s16le', ac=waveform_np.shape[0], ar=sample_rate)
            .run(capture_stdout=True, capture_stderr=True)
        )

        audio = np.frombuffer(out, dtype=np.int16).reshape(waveform_np.shape)
...
def main():
    ...
    output_dir = f'LibriSpeech_audioseal_max_5s'
    dataset_dir = os.path.join(output_dir, 'watermarked')
    os.makedirs(output_dir, exist_ok=True)
    os.makedirs(dataset_dir, exist_ok=True)

    if args.encode:
        encode_audio_files(model, selected_indices, librispeech_test, output_dir, args.max_length)

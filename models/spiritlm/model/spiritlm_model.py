...
import torch
import torchaudio
from models.spiritlm.model.utils import (
...
from models.spiritlm.speech_tokenizer import spiritlm_base, spiritlm_expressive
from transformers import GenerationConfig, LlamaForCausalLM, LlamaTokenizer, set_seed, AutoModelForCausalLM, LogitsProcessorList

_logger = logging.getLogger(__name__)


os.environ["SPIRITLM_CHECKPOINTS_DIR"] = os.getcwd() + "/models/checkpoints"

# Get the base checkpoints directory from environment variable or use the default base path
base_checkpoints_dir = Path(os.getenv("SPIRITLM_CHECKPOINTS_DIR", Path(__file__).parent.parent / "checkpoints"))
...
class Spiritlm:
    ...
    def __init__(self, name: str, device="cpu", **speech_tokenizer_kwargs):
        if Path(name).exists():
            path = name
        else:
            path = CHECKPOINT_DIR / name
        _ensure_model_name(name)
        self.device = device# _set_device_and_return()
        ...
        # original__get_logits_warper = self.model._get_logits_warper
        # print("get logits ")
    ...
    def generate(
        ...
        **kwargs,  # GenerationConfig args can be passing here
        ...
        # # Get only response ids, without the prepended prompt
        # response_ids = generate_ids.sequences[:, inputs["input_ids"].shape[1]:][0]
        ...
        if return_prompt:
            ...
            return decoded_output, generate_ids
        

    def get_audio_embeddings(self):
        return self.speech_tokenizer.hifigan_model.vocoder.model.dict.weight.detach()
